...
emoji: "🫀"
type: tech
topics: ["ai", "graphrag", "jsdoc", "mcp", "typescript"]
published: true
publication_name: "aircloset"
---

:::message
[English Version is here](https://ryantsuji.dev/posts/cortex-product-graph?lang=en)
:::
...
[Part 1（総論）](https://zenn.dev/aircloset/articles/d416342f46f16b)で、cortexというプラットフォーム上で**AIがPRレビューや障害対応を回している**話を書きました。そのflywheelの中心にいるのが**Product Graph**（実装名: `cortex-product-graph`、以下cpg）です。
...
| 4 | Self-Healing + Observability + 自動lint追加 | アラート → AI調査 → 修正PR + 新規lint/型gate → 自動再デプロイで同じ書き方を機械的に弾く | [cortex-self-healing](https://zenn.dev/aircloset/articles/74c7dfab13cea2) |
...
ちょうどその頃、別アプローチで作っていた[DB Graph MCP](https://zenn.dev/aircloset/articles/2731787582881a)は、期待通りに動いていました。
...
![Before / After — コードからの推論だけで作ったグラフvs JSDocをSSoTにしたナレッジグラフ](https://ryantsuji.dev/images/posts/cortex-product-graph/before-after-ssot.png?v=9e12cfe3)
...
別記事（[Agentic Graph RAG](https://zenn.dev/aircloset/articles/341dffee42f454)）でも書いたように、AIとgraphを組み合わせるシステムでは「**ハルシネーションは消えるわけではなく、起こる場所が変わる**」だけです。cpgの場合、その位置はこうなっています:
...
![ビルドパイプライン — JSDoc · Pulumi · docsから1つのナレッジグラフを組み立てる](https://ryantsuji.dev/images/posts/cortex-product-graph/build-pipeline.png?v=8e6184b2)
...
![Product Graph — 4レイヤーが同じノード集合に乗ったナレッジグラフ](https://ryantsuji.dev/images/posts/cortex-product-graph/knowledge-graph-structure.png?v=645ab32f)
...
cpgはMCPサーバー(`cortex-product-graph`)として動いていて、AIから見ると3種類のツールが見えます。これは前回の[Agentic Graph RAG MCPの記事](https://zenn.dev/aircloset/articles/341dffee42f454)で書いた**3層ツール設計**（search / detail / traverse）をそのままcortexの文脈に当てはめたものです:
...
![Runbookパターン — ツール返却値に「次に叩くべきツール呼び出し」が同梱される](https://ryantsuji.dev/images/posts/cortex-product-graph/runbook-flow.png?v=ad0d2d67)
...
次回[**Part 3 ── 自動PRレビュー**](https://zenn.dev/aircloset/articles/91824e55b7fc9c)では、cpgの上に立っている自動PRレビューの全フロー ── GitHub webhookの受信から、AIレビュー / 自動修正 / 自動merge / 並列デプロイまでを取り扱います。Auto Reviewが `usecase: "code-review"` を踏むときに何が起きているか、`[Graph] Critical` 指摘がどう生成されているか、worktreeでAIが修正をかけてpushし直す仕組みなどを書いています。
