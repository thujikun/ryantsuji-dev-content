...
emoji: "🛠️"
type: tech
topics: ["ai", "knowledge-graph", "static-analysis", "typescript"]
published: false
...
[前編](https://zenn.dev/aircloset/articles/a7cf65cc035572)で、46リポジトリに跨る本番システムのコードベースを静的解析で1つのナレッジグラフに統合した話を書きました。 完成はしたものの、 最後に残った課題4つを挙げ、 そのうち一番ヤバいのが **「セマンティック検索ができない (入口問題)」** だと書きました。
...
選択肢は明快でした。 **annotationでコードに直接書き込む**しかない（[連載Part 2](https://zenn.dev/aircloset/articles/f6c990989e60d4)で書いたcortex内部のナレッジグラフと同じアプローチ）。
...
![三層構造でgraphをつくる ── code-graph + db-graph + annotation層](https://ryantsuji.dev/images/posts/code-graph-46-repos-part2/three-layer-architecture.png?v=48e5d21f)
...
- 全リポジトリにAIレビューがまだ浸透しているわけじゃない（cortex内部の全自動レビューは [連載Part 6](https://zenn.dev/aircloset/articles/ea7e0cf2190436) で書いた通り、 cortexモノレポの中だけで成立している話）
...
![main branchとannotation branchを物理分離する運用](https://ryantsuji.dev/images/posts/code-graph-46-repos-part2/annotation-branch-flow.png?v=15a25744)
...
これは [連載Part 6](https://zenn.dev/aircloset/articles/ea7e0cf2190436) で書いた「AIに全コードを通すゲート設計」 の理想を、 既存組織の現実条件に合わせて分離設計したものです。 cortex（社内AIプラットフォーム）は自分が1から組み立てているモノレポなので「全コード必ずAIゲート通過」 が成立しますが、 46リポジトリの本番系では成立しない。 だから理想を諦めるのではなく、 **「人の開発フロー」 と「AIのannotation運用」 を物理的に分けて両方とも回す**、 という選択をしました。
...
そしてもう一つ、 連載 [AIハーネス（Part 1-6）](https://zenn.dev/aircloset/articles/ea7e0cf2190436) との対比で言うと、 こうも整理できます:
