...
![MCPツール利用量ダッシュボード — server別 / tool別 の呼び出し回数 + 平均実行時間](https://ryantsuji.dev/images/posts/ai-observability-practice/mcp-usage-dashboard.png?v=01a508ce)

過去30日で `service-product-graph` が37,458 calls (うちエラー7,024)、 `gws` が19,339 calls、 `db-graph` が17,037 calls ── という形で、 **どのMCPがどれだけ使われているか、 どこで失敗が出てるか**が毎日眺められる状態になっています。 前回の連載 ([code-graph deep dive後編](https://zenn.dev/aircloset/articles/9b63579545582d)) で「annotation graphのMCPが 約50,000 calls / 73 usersに使われている」 と書いた数字も、 ここから引いています。
