...
emoji: "🚀"
type: tech
topics: ["ai", "claude", "gcp", "mcp", "typescript"]
published: true
publication_name: "aircloset"
---

:::message
[English Version is here](https://ryantsuji.dev/posts/17-mcp-servers?lang=en)
:::

## はじめに

以前、全社17DB・994テーブルを安全に横断検索・クエリ実行できる「[DB Graph MCP](https://zenn.dev/aircloset/articles/2731787582881a)」を紹介しました。ありがたいことに反響をいただいたので、今回は**DB Graph以外に社内で運用しているMCPサーバー群**をまとめて紹介します。
...
| カテゴリ | サーバー | 説明 |
|---------|---------|------|
| **データ** | DB Graph | [全社DB辞書 + 実DBクエリ](https://zenn.dev/aircloset/articles/2731787582881a) |
...
詳細は[Sandbox MCPの記事](https://zenn.dev/aircloset/articles/65efe9614f8e73)にまとめています。
...
| サーバー | 対象 | 特徴 |
|---------|------|------|
| DB Graph | [全社DB](https://zenn.dev/aircloset/articles/2731787582881a) | テーブル辞書 + セマンティック検索 + 実DBクエリ + PII匿名化 |
...
それぞれ設計思想が異なり、解決する課題も違います。DB Graphは[DB Graph MCPの記事](https://zenn.dev/aircloset/articles/2731787582881a)、Biz Graphは[Biz Graphの記事](https://zenn.dev/aircloset/articles/7a0b06cb2a35d8)で詳しく紹介しています。
...
**4. 非エンジニアへの開放が一番楽しみ**
Workspace MCPと[Sandbox MCP](https://zenn.dev/aircloset/articles/65efe9614f8e73)により、GitHubアカウントを持たないメンバーでもコードを修正してデプロイできる基盤が整いました。まだ作ったばかりで大きな成果はこれからですが、ここが一番ポテンシャルがあると思っています。
...
[DB Graph MCPの記事](https://zenn.dev/aircloset/articles/2731787582881a)で「テーブル間の繋がり方が特定の人の頭の中にのみ存在する」という課題を紹介しました。MCPサーバー群の全体像を見ると、これはDBに限った話ではないことがわかります。
