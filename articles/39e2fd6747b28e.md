...
`auto-fix` 系PRの内訳は **Service Error Log Detected（本番のエラーログ閾値超え）と Deploy Failed（デプロイ失敗）が大半**。アラートが発火した瞬間にAIが調査を開始、Loki / Product Graph / git blame を辿って根本原因を特定し、修正PRを起票して [Part 3](https://zenn.dev/aircloset/articles/91824e55b7fc9c) の自動レビューに乗せ、APPROVE → 自動マージ → 自動再デプロイで一周まわります。
