...
直前の連載 [code-graph deep dive 後編](https://zenn.dev/aircloset/articles/9b63579545582d) で、 「46リポジトリに跨るコードベースをAIにセマンティック検索できる形にした」 という話を書きました。 その最後に残した課題のひとつが **「動的解析の不在」** でした:
...
今回はその話を、 設計編 (本記事) と [実践編](https://zenn.dev/aircloset/articles/3b8e60fcaab4b7) の2つに分けて書きます。 本記事は **4つの監視対象 (アプリケーション / インフラ / CI / LLM)** を、 それぞれの問いの性質に合わせて違う形に shape する設計判断の話。
...
具体的な計装方針は別記事 [AI Harness 連載Part 4 (Self-Healing)](https://zenn.dev/aircloset/articles/74c7dfab13cea2) で触れたので、 本記事では深掘りしません。 「**標準的な OTel スタックがちゃんと敷かれているか**」 が、 この後の AI 駆動運用の前提になる、 とだけ書いておきます。
...
![CI ログを webhook push でなく post-hoc pull で流す](https://ryantsuji.dev/images/posts/ai-observability-design/ci-log-pull-vs-push.png?v=2bdac5e4)
...
![Claude Code 利用量ダッシュボード — 過去30日で 77.1B トークン、 cache読込が 96% を占める](https://ryantsuji.dev/images/posts/ai-observability-design/cc-usage-dashboard.png?v=d2927e7b)
...
[実践編「AI時代のObservability設計 - PIIとAIの検索性を両立させ、自動修復する」 →](https://zenn.dev/aircloset/articles/3b8e60fcaab4b7)
