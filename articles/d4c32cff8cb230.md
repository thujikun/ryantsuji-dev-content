...
今回はその話を、設計編（本記事）と[実践編](https://zenn.dev/aircloset/articles/3b8e60fcaab4b7)の2つに分けて書きます。本記事は**4つの監視対象**（アプリケーション / インフラ / CI / LLM）を、それぞれの問いの性質に合わせて違う形でObservableにする設計判断の話。
...
そしてmainブランチでfailureが出た瞬間にLogQL alertが発火し、Slackに通知される ── これが[実践編](https://zenn.dev/aircloset/articles/3b8e60fcaab4b7)で扱うSelf-Healingの起点になります。
...
長文をお読みいただきありがとうございました。実践編「[AI時代のObservability設計 - PIIとAIの検索性を両立させ、自動修復する](https://zenn.dev/aircloset/articles/3b8e60fcaab4b7)」も公開済みです。続けてどうぞ。
