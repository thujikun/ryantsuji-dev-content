...
[前編](/posts/code-graph-46-repos)で、46リポジトリに跨る本番システムのコードベースを静的解析で1つのナレッジグラフに統合した話を書きました。 完成はしたものの、 最後に残った課題4つを挙げ、 そのうち一番ヤバいのが **「セマンティック検索ができない (入口問題)」** だと書きました。
...
社内には複数サービスにまたがる大量のDBテーブルがあり、 「どのテーブルがどの業務に使われているか」 はごく一部の人の頭の中にしかない状態でした。 そこで、 ORM定義から静的解析でスキーマを抽出し、 Geminiでテーブル説明文を自動生成、 768次元のベクトルにしてgraphに格納、 自然言語クエリで意味検索できるようにしたのがdb-graphです。
...
**code-graphの中には、 すでに「DBテーブルノード」 が境界ノードとして存在している**ということです（前編で書いた境界ノード3種 = API / Event / DB のうちの1つ）。
...
## ただしAPI / Event の意味付けは別途必要 ── 全関数にannotationを振るのは無理

db-graphの接合でDBコンテキストは入りました。 ただ、 境界ノードはあと2種残っています。 APIとEventです。 これらの意味は誰も静的解析では拾えないので、 何らかの形でコンテキストを注入する必要があります。
...
整理するとこうなります（内部ではservice-product-graph、 略してSPGと呼んでいます。 以降は「上位レイヤー」 と書きます）。

![三層構造でgraphをつくる ── code-graph + db-graph + annotation層](/images/posts/code-graph-46-repos-part2/three-layer-architecture.png)

下から3つの層を積み上げ、 全部をSAME_ENTITYというエッジで接合する構造です。

- **code-graph（構造）** ── 静的解析で抽出した関数 / クラス / 境界ノード（46リポジトリ）
- **db-graph（DBコンテキスト）** ── 1,133テーブルの意味付き
- **annotation層（意図）** ── 境界周辺だけに `@graph-*` で振った意図

そして全体の入口に **MCPサーバー** が立っていて、 ここがAIエージェントから叩かれる唯一の口です。 db-graphには直接接続せず、 上位レイヤーのMCPがプロキシとして動きます。

ノード種は Page / Section / Dialog / Field / Action / Api / Task の7種。 元々は画面中心の設計で `screen-graph` と呼んでいましたが、 backendのApi / Taskまで広げた段階で名前を service-product-graph に変えました。
...
annotation層を作ると決めたあとに直面する制約はこうでした:
...
annotation運用が回るだけでは、 3つのgraph（code-graph / db-graph / annotation層）の **接合の質** は保証されません。 そこで、 graph全体の整合性を機械的に検査するSLOを定義しています。
...
静的解析で抽出したAPI / Page / Taskノードと、 annotation層で書いたAPI / Page / Taskノードは、 別物のノードとして作られます。 同じ意味を持っているはずなのに、 名前 / パス / 識別子の表現が違うので機械的には繋がらない。
...
これを自然言語のままannotation層に投げると、 ベクトル検索で関連ノード（Page / Api / Function / DBテーブル）が事実として返ります。 そこからSAME_ENTITY経由でcode-graphに降りて、 関連する別リポの呼び出し元 / 呼び出し先まで辿れる。 さらにcode-graphのDB境界からdb-graphに降りれば、 関連カラムまで取れる。

つまり、 1つの自然言語クエリで「3つのgraphを横断して、 関連する全コード + 全DBスキーマ」 が取れる状態になりました。
...
特に注目すべきは、 **db-graphには直接接続させない**設計にしていることです。 上位レイヤーのMCPがプロキシとして db-graph を呼び出す形にして、 AIエージェントからは「上位レイヤー1つに聞くだけで全部出てくる」 状態を維持しています。
...
### 4. embeddingのスループット制約

Gemini Embeddingは1テキストずつしか送れない（バッチ送信不可）ので、 大量ノードのembedding生成は時間 / コストのボトルネックになります。 並列度で対応していますが、 根本解はまだです。

### 5. 新リポ追加時の調整負荷

新しいリポジトリが本番系に加わるたびに、 ブリッジの正規化ルールやリポ別パターンの調整が必要です。 前編4番目に書いた「新しい境界パターンが出るたびに独自parserを足す運用負荷」 の、 上位レイヤー側の同型課題です。

## 閉じ: 「捨てた」 のではなく「進化させた」

前編の冒頭で、 cortex（社内AIプラットフォーム）を作り始めた頃に「2ヶ月で捨てた」 と書いたcode-graphの話を扱いました。 でもこの連載を通して振り返ると、 正確には「捨てた」 のではなく **「進化させた」** が正しい表現でした。

進化の正体は、 結局この三層構造に集約されます:

- **code-graph（構造）**
- **db-graph（DBコンテキスト）**
- **境界annotation（意図）**

これらをSAME_ENTITYで接合して、 上位レイヤーのMCPで AI に渡す。 静的解析だけでは届かなかった「意味で引く」 を、 db-graphの成功体験を再利用 + 境界周辺だけannotationを振る最小限戦略で成立させた、 という話でした。
