...
だから、別のアプローチが必要でした。**外側から静的解析でナレッジグラフを作る**。これがcode-graphの出発点です。

## 規模感: 40以上のリポジトリ

対象は2つのgraphに分かれています。

- **air-closet graph（37リポジトリ）**: airCloset（本体）、Spot RENTAL（都度利用型レンタル）、bridge（法人向け）、airCloset Men'sなど、**複数サービスを横断**して1つのgraphになっています。サービス間でデータやAPIの繋がりがあるので、別々のgraphにすると追えなくなる依存関係を1つのgraphにまとめている形です
...
そして実は、**tree-sitterが作れるgraphも、この「読ませる」に毛が生えたくらいの価値しかありません**。同じファイル内、せいぜい同じリポジトリ内の関数呼び出しを構造化するだけ。「目の前のコードを読ませる」に「呼び出し先を辿る」が加わる、その程度です。
