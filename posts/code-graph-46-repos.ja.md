...
境界ノードを取れていれば、AIは「このAPIは他にも〇〇repoで叩かれています」と**事実として**答えられます。AIに推論させるのではなく、**事前に解決済みの事実として渡せる**わけです（抽出時にはTypeScript CompilerやGeminiで推論が入りますが、その結果はgraphに確定値として保存され、後述する境界分析の日次cronでドリフトを翌朝検知できる状態にしています。AIが消費する時点では、検証された事実だけが渡る形になります）。
