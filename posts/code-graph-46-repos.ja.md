...
- **2026-03-16** ─ `refactor: remove db-dictionary from code-graph stack` ── DBスキーマ辞書 (テーブル・カラム定義を引けるレイヤー) を別graphとして独立進化
...
そちらでも最初はcode-graphと同じアプローチを試したのですが、早い段階で見送って、**アノテーションベースのナレッジグラフ**に倒しています。
