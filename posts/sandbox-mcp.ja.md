...
cover: /images/posts/sandbox-mcp.ja.cover.png
---

みなさまこんにちは！エアークローゼットでCTOをしている[辻](https://x.com/RyanAircloset)です。

これまでに[DB Graph MCP](/posts/db-graph-mcp)、[社内MCP群の全体像](/posts/17-mcp-servers)、[Biz Graph MCP](/posts/initiative-graph-rag)と、社内向けに作っているMCPサーバーを順に紹介してきました。
...
- 4層のアクセスゲート（Worker自前OAuth / MCP OAuth / ns分離 / IAM）で**実装の正しさに依存しない安全性**を担保
