...
:::message
**前提**: この3層が成立する大前提として、[Part 2](/posts/cortex-product-graph)で書いた **cpg（コード・docs・DB・インフラを統合したナレッジグラフ）** と、本記事で扱う **Observability スタック**の2つが先に存在している必要があります。
...
なお cortex は **既に100万行を超えるコードベース**で、この規模になるとコード全体をコンテキストとして読み込むことは**AIの側にも不可能**です（人間に不可能なのは言うまでもなく）。grep + ファイル読みだけで影響範囲を辿らせると、AIはコンテキストウィンドウを使い切る前に迷子になります。cpg があって初めてAIは「ある関数の変更が他のどこに波及するか」を1ホップで取りに行ける。小さなrepoなら不要かもしれませんが、ある規模を超えるとcpgはoptionalではなく**必須**です。
...
![Self-Healing 全フロー ── アラート発火から本番復旧まで平均10〜20分](/images/posts/cortex-self-healing/self-healing-flow.png)
...
![cpg を共有基盤に、Self-Healing と 自動レビュー（Sensors）が Guides を育てる](/images/posts/cortex-self-healing/mutual-reinforcement.png)
...
![Self-Healing が動くたびに起きる 5 ステップ ── 再発防止主役のフロー](/images/posts/cortex-self-healing/recurrence-prevention-flow.png)
