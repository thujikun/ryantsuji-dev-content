...
「AIが障害対応まで自動で回す？そんなうまくいくわけない」と思った人へ ── 確かに、**この2つが揃ってない状態では絶対にうまくいきません**。AIが闇雲にエラーログだけを見て本番コードを書き換える運用は、`gh pr create` 1発で事故を起こす速度を上げるだけです。cpg と Observability は、AIに自動修復を任せられる側に立つための**最低条件**です。
