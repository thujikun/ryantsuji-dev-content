...
なおcortexは**既に100万行を超えるコードベース**で、この規模になるとコード全体をコンテキストとして読み込むことは**AIの側にも不可能**です（人間に不可能なのは言うまでもなく）。grep + ファイル読みだけで影響範囲を辿らせると、AIはコンテキストウィンドウを使い切る前に迷子になります。cpg があって初めてAIは「ある関数の変更が他のどこに波及するか」を1ホップで取りに行ける。小さなrepoなら不要かもしれませんが、ある規模を超えるとcpgはoptionalではなく**必須**です。

Part 1で書いたFowlerの分類で言えば、cpgとObservabilityは**Guides側の支える基盤層**。自動自己修復と自動レビュー（Sensors）はこの土台の上で初めて機能する、というのが本記事を通じた構造です。
:::
