...
正直に言うと、AI以前のログPII対策は実質的なROIが薄い領域でした。**ログを読める集合がDBを読める集合にほぼ包含されていた**からです ── DBアクセスを持つエンジニアがログも読める世界では、ログ側だけ固めても総リスクは大きく下がらない。多くの組織で後回しになってきたのは怠慢というより経済的に合理的だった、というのが実態です。

AIはこの包含関係を壊しました。MCP経由でログを引く非エンジニアはDBアクセスを持っていない ── **ログが初めて、DB権限を持たない人がPIIに到達できる経路**になった。これは人数の問題ではなく**信頼境界の再配置**の問題です。加えてAI入力側にも新しい経路が開きます: 外部モデルプロバイダへの送信、prompt injectionによる能動的な抜き取り、AI要約出力を通じてクエリした本人以外の目にPIIが再表出する経路、AI自身の観測ログ (tool-call引数) への写り込み。これまで「**たまたま事故が起きていなかった**」で済んでいた構造的リスクが一気に顕在化する土台条件が、ここで揃いました。
